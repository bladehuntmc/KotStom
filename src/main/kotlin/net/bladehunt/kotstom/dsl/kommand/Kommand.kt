...
data class KommandBuilder(
    ...
    @KommandDSL
    fun subkommand(block: @KommandDSL KommandBuilder.() -> Unit): Kommand = KommandBuilder().apply(block).build().also { subcommands.add(it) }

    @KommandDSL
    fun subcommand(command: Command) = subcommands.add(command)
    ...
    fun build(): Kommand {
        ...
        return Kommand(name).also { kommand ->
            ...
            subcommands.forEach { subcommand ->
                kommand.addSubcommand(subcommand)
            }
        }
    }
}
